...
// Functions define the page height to the window height size
...
var changeColor = function () {
    ...
    $('.container-fluid').css({'background-color':color, "transition":"background-color 1s ease"});
}

var getNewQuote = function () {
    var qID = Math.floor(Math.random() * quotes.frases.length),
        message = quotes.frases[qID],
        author = quotes.author;

    $('#quote').fadeOut(800, function(){
        $(this).html('<i class="fa fa-quote-left"></i> ' + message);
        $('#author').text("- " + author);
        changeColor();
    })
    .fadeIn(800);
    
    $("#tweet").attr("href", "https://twitter.com/intent/tweet?text=" + encodeURIComponent('"' + message + '" - ' + author) + "&hashtags=frases");
}
...
var quotes = { 
    ...
    "frases": 
    [
        "A felicidade (...) é como a água de um recipiente: se a empurramos, ela volta; se a puxamos, ela se afasta.",
        "Haverá verdadeira paz sempre que soubermos agradecer.",
        ...
        "Mesmo que a pessoa consiga ocultar-se dos olhos dos outros, não poderá ocultar-se dos seus próprios olhos.",
        "Orgulho, mania de grandeza, pedantismo e vaidade produzem efeitos negativos. Deus desaprova a presunção.",
        "Não sofra antecipadamente pelo que ainda não passou.",
        "É realmente verdade que gratidão gera gratidão e lamúlia gera lamúria.",
        "Os problemas de um país não são solucionados porque falta amor e sinceridade àqueles que o administram.",
        "Existem mundos maravilhosos e fascinantes, mas existe um mundo infinitamente mais misterioso e atraente: o Céu que está dentro de nós.",
        "Sempre carregue consigo o saco da paciência e costure-o toda vez que romper.",
        ...
        "Ainda que estejamos certos, não devemos desnecessariamente insistir em argumentos a nosso favor.",
        "A verdade é o caminho, o bem a ação e o belo é o sentimento. Desejo ardentemente que todos os cultivem.",
        "Quem está com Deus, não deve lamentar o passado e nem se preocupar com o futuro.",
        "Cada qual deve estabelecer dentro de si um estado de espírito celestial e feliz.",
        ...
        "A verdadeira liberdade respeita a liberdade alheia; portanto, ela tem um limite.",
...
$("document").ready(function () {
    ...
    // Call the fullFill() function on window resize event
    ...
    $('#tweet').on('click', function() {
        window.open(this.href, 'Tweet','left=20,top=20,width=500,height=300,toolbar=0,resizable=0'); 
        return false;
    });    
});